...
class PatrolNode(BasicNavigator):
    ...
    # 订阅和保存图像相关定义
        # 声明一个名为 image_save_path 的参数，并设置其默认值为空字符串。这意味着如果在启动节点时没有通过参数文件或其他方式设置这个参数，它的值将默认为空字符串
        self.declare_parameter('image_save_path', '')
        # 获取之前声明的 image_save_path 参数的值，并将其赋值给 self.image_save_path 变量。这样，你就可以在节点的其他部分使用这个变量来访问参数的值
    ...
    def get_current_pose(self):
        ...
        while rclpy.ok():
            try:
                ...
                """
                seconds=0 为指定的变换时间点，表示请求最新的可用变换（即最近广播的变换）
                seconds=1 指定了等待可用变换的最大时间。如果在1秒内没有找到变换,则会抛出异常或返回失败。
                """
