...
#include "rclcpp_action/rclcpp_action.hpp"      // 导入ROS2的C++客户端库
using NavigationAction = nav2_msgs::action::NavigateToPose;     // 定义消息接口

class NavToPoseClient : public rclcpp::Node {
public:
    using NavigationActionClient = rclcpp_action::Client<NavigationAction>; // 定义导航客户端类型
    ...
    goal_msg.pose.pose.position.x = -1.0f;
    goal_msg.pose.pose.position.y = -1.0f;
    ...
    // 设置移动过程反馈回调函数
        send_goal_options.feedback_callback = [this]( NavigationActionGoalHandle::SharedPtr goal_handle, const std::shared_ptr
        <const NavigationAction::Feedback> feedback) {
